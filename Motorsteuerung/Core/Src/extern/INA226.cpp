...
//  CONSTRUCTOR
//
INA226::INA226(const uint8_t address, I2C_HandleTypeDef* i2c_typedef_)
{
  _address     = address;
...
int INA226::configure(float shunt, float current_LSB_mA, float current_zero_offset_mA, uint16_t bus_V_scaling_e4)
{
  ...
  if((float)(INA226_MAX_SHUNT_VOLTAGE / shunt) < 32768 * current_LSB_mA * 1e-3)
  {
	  maxCurrent = (float)(INA226_MAX_SHUNT_VOLTAGE / shunt);
  }
  else
  {
	  maxCurrent = 32768 * current_LSB_mA * 1e-3;
  }
