...
AS5600::AS5600(I2C_HandleTypeDef* i2c_typedef_)
{
...
bool AS5600::isConnected()
{
  ...
  //return ( _wire->endTransmission() == 0);
	...
	result = HAL_I2C_Master_Transmit(i2c_typedef, _address, NULL, 0, 100);
...
uint8_t AS5600::readReg(uint8_t reg)
{
  ...
  return _data;*/
	...
	// 1️⃣ Schreibe das Register, das du lesen willst
	status = HAL_I2C_Master_Transmit(i2c_typedef, _address, &reg, 1, HAL_MAX_DELAY);
	if (status != HAL_OK)
	{
		// Kein ACK, Busfehler etc.
		return 0;
	}

	// 2️⃣ Lese 2 Bytes vom Sensor
	status = HAL_I2C_Master_Receive(i2c_typedef, _address, data, 1, HAL_MAX_DELAY);
	if (status != HAL_OK)
	{
		return 0;
	}

	// 3️⃣ Kombiniere High- und Low-Byte
...
uint16_t AS5600::readReg2(uint8_t reg)
{
  ...
  return _data;*/
	...
	// 2️⃣ Lese 2 Bytes vom Sensor
	status = HAL_I2C_Master_Receive(i2c_typedef, _address, data, 2, HAL_MAX_DELAY);
	if (status != HAL_OK)
	{
		return 0;
	}
...
uint8_t AS5600::writeReg(uint8_t reg, uint8_t value)
{
  ...
  return _error;*/
    ...
    HAL_StatusTypeDef status = HAL_I2C_Master_Transmit(i2c_typedef, _address, data, 2, HAL_MAX_DELAY);
    return status;  // HAL_OK oder HAL_ERROR / HAL_BUSY / HAL_TIMEOUT
}
...
uint8_t AS5600::writeReg2(uint8_t reg, uint16_t value)
{
  ...
  return _error;*/
    ...
    HAL_StatusTypeDef status = HAL_I2C_Master_Transmit(i2c_typedef, _address, data, 3, HAL_MAX_DELAY);
    return status;  // HAL_OK oder HAL_ERROR / HAL_BUSY / HAL_TIMEOUT
}
