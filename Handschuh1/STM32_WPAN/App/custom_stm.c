...
/* Private typedef -----------------------------------------------------------*/
typedef struct{
  uint16_t  CustomBlserviceHdle;                    /**< BlService handle */
  uint16_t  CustomCharwriteHdle;                  /**< charWrite handle */
  uint16_t  CustomWritecharHdle;                  /**< writeChar handle */
...
#define COPY_BLSERVICE_UUID(uuid_struct)          COPY_UUID_128(uuid_struct,0x00,0x00,0x00,0x00,0xcc,0x7a,0x48,0x2a,0x98,0x4a,0x7f,0x2e,0xd5,0xb3,0xe5,0x8f)
...
#define COPY_WRITECHAR_UUID(uuid_struct)    COPY_UUID_128(uuid_struct,0x00,0x00,0x00,0x01,0x8e,0x22,0x45,0x41,0x9d,0x4c,0x21,0xed,0xae,0x82,0xed,0x19)
    ...
    case HCI_VENDOR_SPECIFIC_DEBUG_EVT_CODE:
      ...
      switch (blecore_evt->ecode)
      {
        case ACI_GATT_ATTRIBUTE_MODIFIED_VSEVT_CODE:
          ...
          if (attribute_modified->Attr_Handle == (CustomContext.CustomWritecharHdle + CHARACTERISTIC_DESCRIPTOR_ATTRIBUTE_OFFSET))
          {
            ...
            switch (attribute_modified->Attr_Data[0])
            {
              ...
              case (!(COMSVC_Notification)):
                ...
                Notification.Custom_Evt_Opcode = CUSTOM_STM_WRITECHAR_NOTIFY_DISABLED_EVT;
              ...
              case COMSVC_Notification:
                ...
                Notification.Custom_Evt_Opcode = CUSTOM_STM_WRITECHAR_NOTIFY_ENABLED_EVT;
                Custom_STM_App_Notification(&Notification);
...
void SVCCTL_InitCustomSvc(void)
{
   ...
   * service_max_attribute_record = 1 for BlService +
   *                                2 for charWrite +
   *                                2 for writeChar +
   *                                1 for writeChar configuration descriptor +
   *                              = 6
   *
  ...
  COPY_BLSERVICE_UUID(uuid.Char_UUID_128);
  ret = aci_gatt_add_service(UUID_TYPE_128,
                             (Service_UUID_t *) &uuid,
                             PRIMARY_SERVICE,
                             max_attr_record,
                             &(CustomContext.CustomBlserviceHdle));
  if (ret != BLE_STATUS_SUCCESS)
  {
    APP_DBG_MSG("  Fail   : aci_gatt_add_service command: BlService, error code: 0x%x \n\r", ret);
  }
  else
  {
    APP_DBG_MSG("  Success: aci_gatt_add_service command: BlService \n\r");
  }
  ...
  ret = aci_gatt_add_char(CustomContext.CustomBlserviceHdle,
                          UUID_TYPE_128, &uuid,
                          SizeCharwrite,
                          CHAR_PROP_WRITE,
   ...
   *  writeChar
   */
  COPY_WRITECHAR_UUID(uuid.Char_UUID_128);
  ret = aci_gatt_add_char(CustomContext.CustomBlserviceHdle,
                          UUID_TYPE_128, &uuid,
                          SizeWritechar,
                          CHAR_PROP_NOTIFY,
                          ...
                          0x10,
                          CHAR_VALUE_LEN_CONSTANT,
                          &(CustomContext.CustomWritecharHdle));
  if (ret != BLE_STATUS_SUCCESS)
  {
    APP_DBG_MSG("  Fail   : aci_gatt_add_char command   : WRITECHAR, error code: 0x%x \n\r", ret);
  }
  else
  {
    APP_DBG_MSG("  Success: aci_gatt_add_char command   : WRITECHAR \n\r");
  }
...
tBleStatus Custom_STM_App_Update_Char(Custom_STM_Char_Opcode_t CharOpcode, uint8_t *pPayload)
{
  ...
  switch (CharOpcode)
  {
    ...
    case CUSTOM_STM_CHARWRITE:
      ret = aci_gatt_update_char_value(CustomContext.CustomBlserviceHdle,
                                       CustomContext.CustomCharwriteHdle,
                                       0, /* charValOffset */
    ...
    case CUSTOM_STM_WRITECHAR:
      ret = aci_gatt_update_char_value(CustomContext.CustomBlserviceHdle,
                                       CustomContext.CustomWritecharHdle,
                                       0, /* charValOffset */
                                       SizeWritechar, /* charValueLen */
                                       (uint8_t *)  pPayload);
      if (ret != BLE_STATUS_SUCCESS)
      {
        APP_DBG_MSG("  Fail   : aci_gatt_update_char_value WRITECHAR command, result : 0x%x \n\r", ret);
      }
      else
      {
        APP_DBG_MSG("  Success: aci_gatt_update_char_value WRITECHAR command\n\r");
      }
...
tBleStatus Custom_STM_App_Update_Char_Ext(uint16_t Connection_Handle, Custom_STM_Char_Opcode_t CharOpcode, uint8_t *pPayload)
{
  ...
  switch (CharOpcode)
  {
    ...
    case CUSTOM_STM_CHARWRITE:
      ...
      ret = Generic_STM_App_Update_Char_Ext(Connection_Handle, CustomContext.CustomBlserviceHdle, CustomContext.CustomCharwriteHdle, SizeCharwrite, pPayload);
    ...
    case CUSTOM_STM_WRITECHAR:
      ...
      ret = Generic_STM_App_Update_Char_Ext(Connection_Handle, CustomContext.CustomBlserviceHdle, CustomContext.CustomWritecharHdle, SizeWritechar, pPayload);
