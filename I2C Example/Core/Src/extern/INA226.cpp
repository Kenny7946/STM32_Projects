...
uint16_t INA226::_readRegister(uint8_t reg)
{
  ...
  return value;*/
	...
	status = HAL_I2C_Master_Transmit(i2c_typedef, _address, &reg, 1, 1000);
	if (status != HAL_OK)
	{
	...
	status = HAL_I2C_Master_Receive(i2c_typedef, _address, data, 2, 1000);
	if (status != HAL_OK)
	{
		return 0;
	}
...
uint16_t INA226::_writeRegister(uint8_t reg, uint16_t value)
{
 /* _wire->beginTransmission(_address);
  ...
  return n;*/
    ...
    HAL_StatusTypeDef status = HAL_I2C_Master_Transmit(i2c_typedef, _address, data, 3, 1000);
